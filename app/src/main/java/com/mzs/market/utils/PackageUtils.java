...
import com.mzs.market.utils.ShellUtils.CommandResult;
...
public class PackageUtils {
    ...
    public interface InstallListener {
        void installSuccess(String path);

        void installFailed(String path);
    }
     ...
     * @param filePath
     */
    public static final void installApkSync(final String filePath, final InstallListener listener) {
        if (listener == null)
            return;
        if (TextUtils.isEmpty(filePath)) {
            listener.installFailed(filePath);
            return;
        }
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                int result = install(MzsApplication.APP, filePath);
                if (result == INSTALL_SUCCEEDED) {
                    AppConfig.uiHandler.post(new Runnable() {
                        @Override
                        public void run() {
                            listener.installSuccess(filePath);
                        }
                    });
                } else {
                    AppConfig.uiHandler.post(new Runnable() {
     ...
     * @param packageName package name of app
     * @param isKeepData whether keep the data and cache directories around
    ...
    public static ArrayList<AppInfo> getUserAppList(Context context) {
        ...
        for (PackageInfo packageInfo : pakageinfos) {
            ...
            appInfo.appName = (str_name);
            String version = packageInfo.versionName;
            appInfo.appVersion = (version);
            Drawable drawable = packageInfo.applicationInfo.loadIcon(pm);
            appInfo.drawable = (drawable);
    ...
    public static AppInfo getUninstallApkInfo(String apkPath) {
        String PATH_PackageParser = "android.content.pm.PackageParser";
        String PATH_AssetManager = "android.content.res.AssetManager";
        AppInfo appInfo = new AppInfo();
        appInfo.appPath = apkPath;
        try {
            ...
            Resources res = MzsApplication.APP.getResources();
            ...
            if (info.labelRes != 0) {
                appInfo.appName = res.getText(info.labelRes).toString();
            }
            if (info.icon != 0) {
                appInfo.drawable = res.getDrawable(info.icon);
            }
            appInfo.packageName = info.packageName;
            appInfo.appSpace = FileUtils.setFileSize(new File(apkPath).length());

            PackageManager pm = MzsApplication.APP.getPackageManager();
